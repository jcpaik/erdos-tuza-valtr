...
#align_import ErdosTuzaValtr.Etv.alpha_beta
...
private theorem mem_imply_nnil {α : Type _} (a : α) {l : List α} (ha : a ∈ l) : l ≠ [] := by
  intro eq <;> subst Eq <;> simp at ha <;> tauto
...
instance decidableIsAlphaCup (a : α) (c : List α) : Decidable (l.IsAlphaCup a c) := by
  rw [is_alpha_cup] <;> infer_instance

theorem alpha_cup_is_cup (c : List α) (c_in_S : c.In S) (c_sorted : c.Sorted (· < ·))
...
def alphaCups' (a : α) : List (List α) :=
  (S.sort (· ≤ ·)).sublists.filterₓ (l.IsAlphaCup a)

def alphaCup' (a : α) : Option (List α) :=
  (l.alphaCups' a).argmax List.length

def alphaCup'_isSome {a : α} (ha : a ∈ S) : Option.isSome (l.alphaCup' a) :=
  by
...
def alpha (a : α) : ℕ :=
  if ha : a ∈ S then (Option.get (l.alphaCup'_isSome ha)).length else 0
...
def cup_length_le_alpha {a : α} {c : List α} (c_in_S : c.In S) (c_sorted : c.Sorted (· < ·))
  ...
  subst eq_c; simp
  have c'_alpha_cup : c' ∈ l.alpha_cups' a :=
    by
    rw [alpha_cups']; simp; constructor
...
instance decidableIsBetaCup (a : α) (c : List α) : Decidable (C.IsBetaCup S a c) := by
  rw [is_beta_cup] <;> infer_instance

def betaCups' (a : α) : List (List α) :=
  (S.sort (· ≤ ·)).sublists.filterₓ (C.IsBetaCup S a)
...
-- one off from actual definition
def beta (a : α) : ℕ :=
  if ha : a ∈ S then (Option.get (C.betaCup'_isSome S ha)).length else 0
...
def cup_length_le_beta {a : α} {c : List α} (c_in_S : c.In S) (c_cup : C.Cup c)
  ...
  have c'_beta_cup : c' ∈ C.beta_cups' S a :=
    by
    rw [beta_cups']; simp; constructor
    · apply List.sublist_of_subperm_of_sorted _ _ (Finset.sort_sorted_lt S)
      apply List.Nodup.subperm
...
theorem Config.Label.alpha_le_beta {a : α} (ha : a ∈ S) : l.alpha a ≤ C.beta S a :=
  by
  rcases l.alpha_cup ha with ⟨c, c_length, c_in, c_sorted, c_chain, c_last⟩
...
theorem slope_tt_inc_beta {a b : α} (sab : l.Slope a b) (ha : a ∈ S) (hb : b ∈ S) (a_le_b : a < b) :
  ...
  by
  rcases C.beta_cup S ha with ⟨c, c_in, ⟨c_cup, c_length⟩, c_last⟩
  rcases List.takeLast' c_last with ⟨c', c_eq⟩
  rw [Nat.lt_iff_add_one_le, ← add_le_add_iff_right 1]
  set d := c ++ [b] with def_d
