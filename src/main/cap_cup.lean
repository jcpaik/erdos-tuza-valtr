...
lemma has_cap2_cup2 {S : finset α} (hS : 1 < S.card) : 
  C.has_ncap 2 S ∧ C.has_ncup 2 S :=
begin
  set l := S.sort (≤) with eq_l,
  have hl : 2 ≤ l.length := by rw eq_l; simp; exact hS,
  rcases list.take_head2 hl with ⟨a, b, t, eq_ab⟩,
  have sorted := finset.sort_sorted_lt S, rw ←eq_l at sorted,
  have a_lt_b : a < b := 
    by rw eq_ab at sorted; simp at sorted; tauto,
...
theorem binom_eq (a b : ℕ) :
  (a + b + 2).choose (a + 1) = 
  (a + b + 1).choose a + (a + b + 1).choose (a + 1) :=
rfl

theorem cap_cup (a b : ℕ)
  (S : finset α) (hS : nat.choose (a+b) a < S.card) :
  C.has_ncap (a+2) S ∨ C.has_ncup (b+2) S :=
begin
  revert a b S hS, refine nat.pincer_recursion _ _ _,
  -- case b = 0
  { simp, intros a S hS, right, exact (C.has_cap2_cup2 hS).right },
  -- case a = 0
  { simp, intros a S hS, left, exact (C.has_cap2_cup2 hS).left },
  -- diagonal induction
  { intros a b,
    ...
    have eq_sz : sz_a1b1 = sz_ab1 + sz_a1b :=
      by rw [eq_sz_ab1, eq_sz_a1b, eq_sz_a1b1]; ring_nf,
    -- numerical details now not relevant
    clear eq_sz_ab1 eq_sz_a1b eq_sz_a1b1,
    ring_nf, intros hab1 ha1b S hS,

    set is_start_of_cap : α → Prop :=
      λ p, ∃ c, C.cap c ∧ c.in S ∧ c.length = a + 2 ∧ p ∈ c.head' 
      with def_is_start_of_cap,
    set T := finset.filter is_start_of_cap S with def_T,
    have eq_card : (S \ T).card + T.card = S.card := begin
      apply finset.card_sdiff_add_card_eq_card,
      rw def_T, exact S.filter_subset is_start_of_cap,
    end,
    have sz_cases : sz_ab1 < (S \ T).card ∨ sz_a1b < T.card := 
      by by_contra'; linarith,
    cases sz_cases,
    -- case sz_ab1 < (S \ T).card
    { cases hab1 (S \ T) sz_cases with hcap hcup,
      rcases hcap with ⟨c, ⟨c_cap, c_length⟩, c_in⟩,
      { have c_nnil : c ≠ [] :=
          by intro eq_c; subst eq_c; cases c_length,
        rcases list.take_head c_nnil with ⟨ch, ct, eq_c⟩,
      ...
      { right, 
        exact has_ncup_supset (finset.sdiff_subset S T) hcup }, },
    -- case sz_a1b < T.card
    cases ha1b T sz_cases with hcap hcup,
    { left, refine has_ncap_supset _ hcap,
      rw def_T, simp, },
    { rcases hcup with ⟨cl, ⟨cl_cup, cl_length⟩, cl_in_T⟩,
      have cl_sz2 : 2 ≤ cl.length := by rw cl_length; simp,
      rcases list.take_last2 cl_sz2 with ⟨p, q, cl', eq_cl⟩, clear cl_sz2,
      ...
      rcases list.take_head2 cr_sz2 with ⟨q, r, cr', eq_cr⟩, clear cr_sz2,
